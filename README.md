...
```elixir
# Encode source data for a block of K symbols
...
{:ok, state} = Raptorq.encode(payload, 500)
...
padding). The ISIs are the **ESI** from RFC 6330 — any non-negative integer up
to 2²⁰−1 works, giving over a million unique encoded symbols per block. No
return channel needed.

## Performance

| Block Size | Dense Solver | Sparse Solver (solver5) |
|------------|-------------|------------------------|
| K=10  L=27 |  1ms | <1ms |
| K=200 L=233 | 322ms | TBD |
| K=500 L=558 | 3.7s | TBD |
| K=1000 L=1071 | 23s | TBD |
